...
class AIGCIdentification extends StatefulWidget {
  @override
  _AIGCIdentificationState createState() => _AIGCIdentificationState();
}

class _AIGCIdentificationState extends State<AIGCIdentification> {

  late FileProvider2 _fileSelector;
  ...
  AIGCIndetificationResult? _analysisResult;
  List<dynamic> list = [];
  ...
  Future<void> _analyzeText(List<String> content) async {
    ...
    try {
      final result = await analyzeTextRisk(content, date, province, area,'ai');
      setState(() {
  ...
  Widget build(BuildContext context) {
    _fileSelector = Provider.of<FileProvider2>(context, listen: false);
    return Stack(
      children: [
        Positioned(
          top: 20,
  ...
  Widget build(BuildContext context) {
      return Expanded(
        ...
        child: Align(
          ...
          child: ConstrainedBox(
            constraints: BoxConstraints(maxWidth: MediaQuery.of(context).size.width - 300), // 设置最大宽度
            child: Padding(
...
class AIMessage extends StatefulWidget {
  final AIGCIndetificationResult? message;
  AIMessage({required this.message});
      ...
      return Expanded(
        ...
        child: Align(
          ...
          child: ConstrainedBox(
            constraints: BoxConstraints(maxWidth: MediaQuery.of(context).size.width - 300), // 设置最大宽度
            child: Padding(
              padding: EdgeInsets.only(right: 300), // 设置左边距
              child: Container(
                ...
                child: Column(
                  ...
                  children: [
                   Text(
                    ...
                    Center(
                      child:Text(
                          widget.message?.confidence.toString() ?? 'Unknown',
                          // 'a',
                          style: 
                          TextStyle(
                            fontSize: 20, 
                            color: getScoreColor(widget.message?.confidence.toDouble() ?? 0.0), 
                            fontFamily: 'MiSans',
                            fontWeight: FontWeight.w600,
                          ),
                    ...
                    Center(
                      child:Text(
                        widget.message != null
                            ? '推理结果: ${widget.message!.is_ai_generated ? 'AI生成' : '非AI生成'}'
                            : '未知',
                    ...
                    Center(
                      child: GradientRoundedProgressBar(
                        progress: widget.message?.confidence.toDouble() ?? 0.0,
                        color1: getScoreColor((widget.message?.confidence.toDouble() ?? 0.0) - 0.3),
                        color2: getScoreColor(widget.message?.confidence.toDouble() ?? 0.0),
                        ),
                    ),

                    ...ReasonList(adviceList: widget.message!.reasons,).get(context),
...
class _ImageUploadButtonState extends State<ImageUploadButton> {
  ...
  late FileProvider2 _imageSelector;

  @override
  void initState() {
    super.initState();
    _imageSelector = Provider.of<FileProvider2>(context, listen: false);
...
class ImagePickerController with ChangeNotifier {
  String base64Image = '';
  FileProvider2? fileProvider; // Make fileProvider nullable.
  ...
  void addFileIcon(BuildContext context, String path, String type,String name,String imageBase64,String suffix) {
    ...
    final _aIGCIdentificationState = context.findAncestorStateOfType<_AIGCIdentificationState>();
    if (_aIGCIdentificationState != null) {
      _aIGCIdentificationState.setState(() {
        _aIGCIdentificationState._isVisibleInstance = true;
      });
    }
  }
...
String getFileExtension(String name) {
  ...
  Future<String> readAsDataUrl(BuildContext context,html.File file) async {
    Provider.of<FileProvider2>(context, listen: false).setFileUploadingState(true);
